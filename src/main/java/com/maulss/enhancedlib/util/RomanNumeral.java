...
public final class RomanNumeral {
	...
	public RomanNumeral(String roman) {
		if (roman.length() == 0) try {
			throw new EnhancedException(new NumberFormatException("An empty string does not define a Roman numeral."));
		...
		if (arabic > 3999) try {
			throw new EnhancedException(new NumberFormatException("Roman numeral must have value 3999 or less."));
		} catch (EnhancedException e) {
			e.printStackTrace();
		}
